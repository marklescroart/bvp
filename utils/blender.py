...
def set_up_group(ObList=None,Scn=None):
    ...
    if len(p)>1:
        ...
        p = p[0]
    no_parent = [o for o in ObList if o.parent or 'ChildOf' in o.constraints.keys()]
    if p:
        for o in no_parent:
            GrabOnly(o)
...
def add_group(name, fname, fPath=bvp.Settings['Paths']['LibDir']+'/Objects/', proxy=True):
    ...
    As group is added, this function assures that the parent object in the group is selected (I HOPE)
    ''' 
    ...
    if name in bpy.data.groups:
        ...
        G = bpy.context.object
        if G is None:
            #no_parent = [g for g in bpy.data.groups[name].objects if g.parent is None]
            #if len(no_parent)>1:
            #    raise Exception("WTF! More than one no-parent object in imported group!")
            #G = no_parent[0]
            #G.select = True
            #bpy.context.scene.objects.active = G
            G = bpy.data.groups[name]
    return G
