...
class Sky(MappedClass):
    ...
    def __init__(self, name='default_outdoor', light_location=None, light_type=None, light_rotation=None,
                 ...
                 is_cycles=None, world_params=None, world_lighting=None, type='Sky', dbi=None, _id=None, _rev=None):
    ...
    def place(self, number=0, scn=None, scale=None, proxy=False):
        ...
        if not self.name in [None, 'default_indoor', 'default_outdoor', 'none']:
            ...
            sky_ob = utils.blender.add_group(
                self.name, self.fname, self.path, proxy=proxy)
            ...
            if proxy:
                for o in sky_ob.dupli_group.objects:
                    ...
                    # , object=sky_ob.name, type=o.name)
            ...
            if self.materials is not None:
                if proxy:
                    raise ValueError(
                        "Can't change materials of proxy objects. Use proxy=False.")
                self.apply_materials(skg[0], self.materials)
            # Rename lamps
            for l in lamp_ob:
                l.name = 'BG_Lamp%04d' % (number)
            # Add world
            # Relies on world being named same thing as sky group... Could be problematic, but anything else is a worse pain
            world = self.add_world()
            update()
        else:
            # Clear other lamps
            ...
            if self.light_type is not None:
                ...
                if bpy.app.version < (2, 80, 0):
                    new_lamp = bpy.data.lamps.new(
                        'Default lamp', self.light_type)
                else:
                    new_lamp = bpy.data.lights.new(
                        'Default lamp', self.light_type)
    ...
    def add_world(self, scn=None):
        ...
        bpy.ops.wm.append(
            # i.e., directory WITHIN .blend file (Scenes / Objects / World / etc)
            directory=os.path.join(self.path, self.fname)+"/World/",
            # local filepath within .blend file to the world to be imported
            filepath="//"+self.fname+"/World/"+self.name,
            # "filename" being the name of the data block, i.e. the name of the world.
            filename=self.name,
            link=False,
            #relative_path=False,
            autoselect=True)
    ...
    def apply_materials(self, bpy_grp, materials):
        ...
        if len(materials) > 1:
            raise NotImplementedError(
                'Multiple material assignment not working yet.')
    ...
    def __repr__(self):
        ...
        if self.semantic_category:
            ...
            for s in self.semantic_category[1:]:
                S += ', %s' % s
            S += '\n'
        if self.n_vertices:
