...
class Action(MappedClass):
	...
	'''
	def __init__(self, name='DummyAction', fname=None, armature='mixamo_human', wordnet_label=None, wordnet_frames=None, 
		...
		# Allow dbi to be a list of parameters to create a DBInterface. This helps with rendering on cluster 
		# computers; dictionaries are easier to store/pass as arguments than objects. 
		if isinstance(self.dbi, dict):
			self.dbi = DBInterface(**self.dbi)
	...
	def from_blender(cls, context):
		...
		dbi = DBInterface(port=Settings['db']['port'], dbname=wm.active_db)
